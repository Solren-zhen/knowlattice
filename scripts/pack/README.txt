...
二、导入随包资料（仅在明确指定数据的离线包中存在）
  1. 在应用左侧目录栏，点右上角的「⋯」（更多操作）
  2. 选择「从备份 .json 恢复」
  3. 选择本文件夹里的  data/notes-and-qbanks.json
  4. 提示「已恢复 N 篇笔记」后，目录树里就会出现全部笔记；
     如果这是默认的应用本体包，请跳过本节：包不携带任何用户数据。

  注意：默认包的数据文件为空。题库备份仅可在取得再分发授权或适用法定例外后，
  通过 --private-data 制作非公开分发包；「私下分发」本身不代表已获得版权许可。
...
五、目录结构
  app/                        应用本体（静态网页，含全部功能）
  data/notes-and-qbanks.json  明确指定时才包含的私下数据备份；默认为空
